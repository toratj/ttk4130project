...
air between blower and nitrification tank."),
  ...
  partial model ASM1base "Base class of WWTP modelling by ASM1"
    ...
    Real mu_h;// = 4.0 "Maximum heterotrophic growth rate f(T) [day^-1]";
    Real b_h; //= 0.3 "Heterotrophic decay rate f(T) [day^-1]";
    Real mu_a;// = 0.5 "Maximum autotrophic growth rate f(T) [day^-1]";
    //Real K_nh "Half-saturation (auto. growth) f(T) [g NH-N/m3]";
    Real b_a;// = 0.05 "Autotrophic decay rate f(T) [day^-1]";
    Real k_a;// = 0.05 "Ammonification rate f(T) [m3/(g COD day)]";
    Real k_h;// = 3.0 "Maximum specific hydrolysis rate f(T) [g Xs/(g Xbh COD day)]";
    Real K_x;// = 0.1 "Half-saturation (hydrolysis) f(T) [g Xs/(g Xbh COD)]";
