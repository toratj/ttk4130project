...
package Examples "Demonstration examples of the components of the ASM1 library"
  ...
  class SmallPlant "Small WWTP Configuration"
    ...
    ASM1.EffluentSink Effluent annotation (Placement(transformation(extent={{150,-27},
              {170,-7}})));
    ASM1.SludgeSink WasteSludge annotation (Placement(transformation(extent={{149,-50},
              {169,-30}})));
    ASM1.divider2 divider annotation (Placement(transformation(extent={{56,-5},
              {76,15}})));
    ASM1.nitri tank3(V=1333) annotation (Placement(transformation(extent={{-6,
              -6},{14,14}})));
    ASM1.nitri tank2(V=1333) annotation (Placement(transformation(extent={{-28,-6},
              {-8,14}})));
    ASM1.deni tank1(V=3000) annotation (Placement(transformation(extent={{-77,-6},
              {-57,14}})));
    ...
    Modelica.Blocks.Sources.CombiTimeTable CombiTableTime(
      ...
      tableOnFile=("Inf_dry") <> "NoName") annotation (Placement(transformation(
            extent={{-114,78},{-94,98}})));
    ASM1.WWSource WWSource annotation (Placement(transformation(extent={{-84,78},
              {-64,98}})));
    ...
    ASM1.sensor_O2 sensor_O2 annotation (Placement(transformation(extent={{25,24},
              {45,44}})));
    Modelica.Blocks.Math.Feedback Feedback annotation (Placement(transformation(
            extent={{124,41},{144,61}})));
    Modelica.Blocks.Continuous.PI PI1(T=0.001, k=500, initType=Modelica.Blocks.Types.Init.InitialState)
      annotation (Placement(transformation(extent={{150,41},{170,61}})));
    ...
    Modelica.Blocks.Sources.Step OxygenSetpoint(height=1.5)
      annotation (Placement(transformation(extent={{99,41},{119,61}})));
    WasteWater.ASM1.SecClar.Krebs.SecClarModKrebs Settler annotation (Placement(
          transformation(extent={{110,-3},{130,17}})));
    WasteWater.ASM1.sensor_TSS sensor_TSS1 annotation (Placement(transformation(
            extent={{94,15},{111,31}})));
    ASM1.deni tank4(V=3000) annotation (Placement(transformation(extent={{-52,-6},
              {-32,14}})));
    ASM1.nitri tank5(V=1333) annotation (Placement(transformation(extent={{20,-4},
              {40,16}})));
    ASM1.blower blower3(Q_max=34574.2654508612) annotation (Placement(
          transformation(extent={{19,-62},{39,-42}})));
    ...
    WasteWater.ASM1.sensor_NO sensor_NO
      annotation (Placement(transformation(extent={{-51,28},{-31,48}})));
  ...
  equation
    ...
    connect(mixer.In1, WWSource.Out) annotation (Line(points={{-104,35.5},{-104,
            74},{-68,74},{-68,81},{-64.2,81}}));
    connect(CombiTableTime.y, WWSource.data)
      annotation (Line(points={{-93,88},{-90,88},{-83,88}}));
    connect(blower2.AirOut, tank3.AirIn) annotation (Line(points={{3,-42},{3,
            -24},{4,-24},{4,-5.8}}));
    connect(Feedback.y, PI1.u) annotation (Line(points={{143,51},{148,51}}));
    connect(divider.Out2, RecyclePump.In) annotation (Line(points={{76,3.5},{
            106,3.5},{106,-16.7},{-76,-16.7}}));
    connect(RecyclePump.Out, mixer.In3) annotation (Line(points={{-96,-22.8},{
            -104,-22.8},{-104,27.5}}));
    connect(ReturnPump.Out, mixer.In2) annotation (Line(points={{58,-34.8},{
            15.5,-34.8},{15.5,-35},{-111,-35},{-111,31.5},{-104,31.5}}));
    connect(sensor_O2.So, Feedback.u2)
      annotation (Line(points={{44.8,34},{134,34},{134,43}}));
    connect(Temperature.y, tank1.T)
      annotation (Line(points={{-81.4,56},{-71,56},{-71,8},{-77,8}}, color={0,0,
            255}));
    connect(Temperature.y, tank3.T) annotation (Line(points={{-81.4,56},{-39,56},
            {-6,56},{-6,8}},                       color={0,0,255}));
    connect(OxygenSetpoint.y, Feedback.u1)
      annotation (Line(points={{120,51},{126,51}},
                                                 color={0,0,255}));
    connect(Constant1.y, blower1.u) annotation (Line(points={{-46,-77},{-7.2,
            -77},{-7.2,-55},{-13.2,-55}}, color={0,0,255}));
    connect(WastePump.Out, WasteSludge.In) annotation (Line(points={{141,-41.2},
            {143,-41.2},{143,-41},{145,-41},{145,-41.2},{149,-41.2}}));
    connect(WastePump.u, Constant2.y)
      annotation (Line(points={{122.1,-41.5},{108,-41.5},{108,-57},{105,-57}},
                                                                     color={0,0,
            255}));
    connect(tank2.Out, tank3.In) annotation (Line(points={{-8,4},{-7,4},{-6,4}}));
    connect(blower1.AirOut, tank2.AirIn) annotation (Line(points={{-24,-42},{
            -24,-5.8},{-18,-5.8}}));
    connect(Settler.Effluent, Effluent.In) annotation (Line(points={{130.2,12.7},
            {140,12.7},{140,-15},{150,-15}}));
    connect(Settler.Return, ReturnPump.In) annotation (Line(points={{117,-2.6},
            {117,-28.7},{78,-28.7}}));
    connect(WastePump.In, Settler.Waste) annotation (Line(points={{121,-47.3},{
            114,-47.3},{114,-30},{123,-30},{123,-2.6}}));
    connect(sensor_NH1.In, Settler.Effluent) annotation (Line(points={{134,16},
            {134,12.7},{130.2,12.7}}));
    connect(sensor_NO1.In, Settler.Effluent) annotation (Line(points={{151,16},
            {151,12.7},{130.2,12.7}}));
    connect(sensor_TKN1.In, Settler.Effluent) annotation (Line(points={{167,15},
            {167,12.7},{130.2,12.7}}));
    connect(sensor_COD1.In, Settler.Effluent) annotation (Line(points={{167,-4},
            {167,12.7},{130.2,12.7}}));
    connect(Constant2.y, ReturnPump.u) annotation (Line(points={{105,-57},{110,
            -57},{110,-34.5},{76.9,-34.5}},color={0,0,255}));
    connect(divider.Out1, Settler.Feed) annotation (Line(points={{76,7.6},{
            106.5,7.6},{106.5,8.4},{110,8.4}}));
    connect(sensor_TSS1.In, divider.Out1) annotation (Line(points={{102.5,15},{
            102.5,10.6},{76,10.6},{76,7.6}}));

    connect(tank4.Out, tank2.In) annotation (Line(
        points={{-32,4},{-28,4}},
        color={0,0,0},
        smooth=Smooth.None));
    connect(tank1.Out, tank4.In) annotation (Line(
        points={{-57,4},{-52,4}},
        color={0,0,0},
        smooth=Smooth.None));
    connect(tank3.Out, tank5.In) annotation (Line(
        points={{14,4},{17,4},{17,6},{20,6}},
        color={0,0,0},
        smooth=Smooth.None));
    connect(tank5.Out, divider.In) annotation (Line(
        points={{40,6},{48,6},{48,5.3},{56,5.3}},
        color={0,0,0},
        smooth=Smooth.None));
    connect(sensor_O2.In, tank5.MeasurePort) annotation (Line(
        points={{35,24},{35,10.5},{35.5,10.5}},
        color={0,0,255},
        smooth=Smooth.None));
    connect(blower3.AirOut, tank5.AirIn) annotation (Line(
        points={{28,-42},{29,-42},{29,-3.8},{30,-3.8}},
        color={0,0,0},
        smooth=Smooth.None));
    connect(blower3.u, PI1.y) annotation (Line(
        points={{38.8,-55},{56,-55},{56,-74},{177,-74},{177,51},{171,51}},
        color={0,0,127},
        smooth=Smooth.None));
    connect(tank2.T, tank3.T) annotation (Line(
        points={{-28,8},{-28,56},{-6,56},{-6,8}},
        color={0,0,127},
        smooth=Smooth.None));
    connect(tank5.T, tank3.T) annotation (Line(
        points={{20,10},{7,10},{7,56},{-6,56},{-6,8}},
        color={0,0,127},
        smooth=Smooth.None));
    connect(tank4.T, tank3.T) annotation (Line(
        points={{-52,8},{-52,56},{-6,56},{-6,8}},
        color={0,0,127},
        smooth=Smooth.None));
    connect(tank4.MeasurePort, sensor_NO.In) annotation (Line(
        points={{-36.5,8.5},{-36.5,15.25},{-41,15.25},{-41,28}},
        color={0,0,0},
        smooth=Smooth.None));
    connect(blower2.u, blower1.u) annotation (Line(
        points={{13.8,-55},{17,-55},{17,-77},{-7.2,-77},{-7.2,-55},{-13.2,-55}},
        color={0,0,127},
        smooth=Smooth.None));

    connect(sensor_NO.Sno, Feedback1.u2) annotation (Line(
        points={{-31.2,38},{-25,38}},
        color={0,0,127},
        smooth=Smooth.None));
    connect(Feedback1.u1, Constant3.y) annotation (Line(
        points={{-17,46},{-17,76}},
        color={0,0,127},
        smooth=Smooth.None));
    connect(Feedback1.y, RecyclePump.u) annotation (Line(
        points={{-17,29},{-17,-22.5},{-77.1,-22.5}},
        color={0,0,127},
        smooth=Smooth.None));
    annotation (
      Diagram(coordinateSystem(
          preserveAspectRatio=false,
          extent={{-120,-90},{180,105}},
          grid={1,1}), graphics),
